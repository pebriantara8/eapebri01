...
void OnTick()
{
  double openPriM5 = iOpen(Symbol(), PERIOD_M5, 1);
  double closePriM5 = iClose(Symbol(), PERIOD_M5, 1);
  double highPriM5 = iHigh(Symbol(), PERIOD_M5, 1);
  double lowPriM5 = iLow(Symbol(), PERIOD_M5, 1);

  double openPriM1 = iOpen(Symbol(), 0, 1);
  double closePriM1 = iClose(Symbol(), 0, 1);
  double highPriM1 = iHigh(Symbol(), 0, 1);
  double lowPriM1 = iLow(Symbol(), 0, 1);
  ...
  // datetime time1 = D'2025.09.04 22:13';
  // datetime time2 = D'2025.09.04 23:59';
  
  datetime time1 = D'2025.09.03 00:00';
  ...
  if (TimeCurrent() > time1)
  {
      ...
      if(PositionsTotal() <= 0){
        ...
        // CEK APAKAH BODY CANDLE M1 MELEWATI LOWHIGH CANDLE M5
        // CEK BUY
        if(highPriM5>openPriM1 && highPriM5<closePriM1){
          // KONFIRMASI BODY CANDLE SEBELUMNYA TIDAK LEBIH BESAR BAGIAN BAWAH
          if((closePriM1-highPriM5)>(highPriM5-openPriM1)){
            trade.Buy(
              0.01, // lots
              NULL, // current symbol
              Ask,  // buy price
              ...
              0,    // take profit
              "Buy ea");
              printf("BUUYYYYYYYY HIGHPRI="+highPriM5);
              printf("BUUYYYYYYYY OPENM1="+openPriM1);
              printf("BUUYYYYYYYY CLOSEM1="+closePriM1);
          }
        }

        // if(lowPriM5<openPriM1 && lowPriM5>closePriM1){
        //   trade.Sell(
        //     0.01, // lots
        //     NULL, // current symbol
        //     Ask,  // buy price
        ...
        //     0,    // take profit
        //     "Sell ea");
        //     printf("SELLLLL LOWHPRI="+lowPriM5);
...
void tambahPosisi(double lotA,double minusA){
  ...
  if(!PositionGetTicket(0)) {
    ...
    if(pP <= minusA){
      ENUM_POSITION_TYPE posisiTipe = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
      if(posisiTipe==0){
        trade.Buy(
          lotA, // lots
          NULL, // current symbol
