...
input double pointTP = 100;
input double pointSL = 110;
input double pointSLFirstRunning = 100;
input double pointSLDiffFirst = 10;
  ...
  // if (TimeCurrent() > time1 && TimeCurrent() < time2)
  {
    ...
    if (PositionsTotal() <= 0)
    {
      ...
      if (OrdersTotal() <= 0)
      {
        trade.BuyStop(
            lotA,                           // lots
            Ask + pointChangeOpen * _Point, // sell price
            NULL,                           // current symbol
            Ask,                            // stop loss
            // Ask + (pointChangeOpen + pointTP) * _Point, // take profit
            0,              // take profit
        ...
        trade.SellStop(
            lotA,                           // lots
            Bid - pointChangeOpen * _Point, // sell price
            NULL,                           // current symbol
            Bid,                            // stop loss
            0,                              // take profit
            // Bid - (pointChangeOpen + pointTP) * _Point, // take profit
            ORDER_TIME_GTC, // order lifetime
            0,              // order expiration time
            "Sell Stop By p");
      }
    ...
    else
    {
      ...
      else
      {
        ...
        if (n != nUpdate)
        {
          ...
          if (posisiType == 0 && posisiCurrentPrice >= posisiOpenPrice + pointSLFirstRunning * _Point)
          {
          ...
          if (posisiType == 1 && posisiCurrentPrice <= posisiOpenPrice - pointSLFirstRunning * _Point)
          {
        ...
        {
          // JIKA BLM ADA UPDATE MODIF POSISI
          CTrade sTrade;
          // JIKA POSISI BUY
          if (posisiType == 0)
          {
            ...
            if (posisiCurrentPrice > posisiOpenPrice + (pointSLFirstRunning * _Point))
            {
              ...
              SLNew = posisiCurrentPrice - pointSLDiffFirst * _Point;
              if (SLNew > posisiSL)
              {
                sTrade.PositionModify(
                    PositionGetInteger(POSITION_TICKET),
